...
def wishMe():
    ...
    if(hour>=0) and (hour<=12) and ('AM' in t):
        speak(f"Good morning Aditi, it's {day} and the time is {t}")
    elif(hour>=12)  and (hour<=16) and ('PM' in t):
        speak(f"Good afternoon Aditi, it's {day} and the time is {t}")
    else:
        speak(f"Good evening Aditi, it's {day} and the time is {t}")
...
def browsing(query):
    if 'google' in query:
        ...
        webbrowser.open(f"{s}")
    #elif 'edge' in query:
    #    speak("oepening your microsoft edge")
    #    os.startfile()
